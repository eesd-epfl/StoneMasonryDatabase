...
export function createCSVArray(data){
    ...
    for (let i = 0; i<activePlotData.length; i++){
        if(activePlotData[i]['ID']>99){
            selectedCurvesFilePaths.push(config.curvesFolderPath+"curve"+activePlotData[i]['ID'] + ".csv");
        } else if (activePlotData[i]['ID']>9){
            selectedCurvesFilePaths.push(config.curvesFolderPath+"curve0"+ activePlotData[i]['ID']+ ".csv");
        } else{
            selectedCurvesFilePaths.push(config.curvesFolderPath+"curve00"+ activePlotData[i]['ID'] + ".csv"); 
        }
    }
    for (let i = 0; i<selectedCurvesFilePaths.length; i++){
        divName.push(selectedCurvesFilePaths[i].split('/')[2].split('.')[0]);
    }
